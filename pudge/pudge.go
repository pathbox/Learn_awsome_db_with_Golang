...
// Db represent database 每个数据库的结构
type Db struct {
	sync.RWMutex          // 读写锁
	name         string   // Db名字
	fk           *os.File // 存储key的磁盘持久化文件
	fv           *os.File // 存储value的磁盘持久化文件
	keys         [][]byte // key byte数组
	vals         map[string]*Cmd
	cancelSyncer context.CancelFunc
	storemode    int // 存储模式  内存模式 文件磁盘模式
}

// Cmd represent keys and vals address
type Cmd struct {
	Seek    uint32 // val的地址位置
	Size    uint32 // 大小
	KeySeek uint32 // key的地址索引位置
	Val     []byte // 值byte数据
}
...
func newDb(f string, cfg *Config) (*Db, error) {
	fmt.Println("newdb:", f, cfg.StoreMode)
	var err error
	db := new(Db)
	db.Lock()
	defer db.Unlock()
	...
	if err != nil {
		if os.IsNotExist(err) {
			if filepath.Dir(f) != "." {
				err = os.MkdirAll(filepath.Dir(f), os.FileMode(cfg.DirMode)) // 以db name 创建目录
				if err != nil {
					return nil, err
				}
			}
	...
	// read keys
	buf := new(bytes.Buffer)        // new bytes.Buffer
	b, err := ioutil.ReadAll(db.fk) // 读取db fk整个文件
	if err != nil {
		return nil, err
	}
	buf.Write(b) // 写入到 buffer
	var readSeek uint32
	for buf.Len() > 0 {
		...
		if db.storemode == 2 {
			...
			db.fv.ReadAt(cmd.Val, int64(seek)) // 从文件中读取存储的所有val byte数据
		}
...
func (db *Db) lessBianry(i, j int) bool {
...
// KeyToBinary return key in bytes v接口值转换为[]byte数据
